...
public:
	...
	enum HTTP_CODE
	{
		NO_REQUESTION,		//NO_REQUESTION是代表请求不完整,需要客户继续输入;
		GET_REQUESTION,		//GET_REQUESTION代表获得并且解析了一个正确的HTTP请求;
		BAD_REQUESTION,		//BAD_REQUESTION是HTTP请求语法不正确；
		FORBIDDEN_REQUESTION,//FORBIDDEN_REQUESTION是代表访问资源的权限有问题;
		FILE_REQUESTION,	//FILE_REQUESTION代表GET方法资源请求；
		INTERNAL_ERROR,		//INTERNAL_ERROR代表服务器自身问题；
		NOT_FOUND,			//NOT_FOUND代表请求的资源文件不存在；
		DYNAMIC_FILE,		//DYNAMIC_FILE表示是一个动态请求；
		POST_FILE			//POST_FILE表示获得一个以POST方式请求的HTTP请求		
	};
	 /*HTTP请求解析的状态转移。
	 HEAD表示解析头部信息;
	 REQUESTION表示解析请求行*/
...
private:
	...
	HTTP_CODE requestion_analyse(char &check_index,int &read_buf_len);	//http请求行解析
	HTTP_CODE do_post();				//对post请求中的参数进行解析
	HTTP_CODE do_file();				//对GET请求方法中的url 协议版本的分离
	void modfd(int epfd,int sock,int ev);//改变socket状态
...
bool http_coon::successful_respond()	//200
{
	m_flag = false;
...
bool http_coon::bad_respond()	//400
{
	...
	sprintf(filename,"/home/zhangxingqi/Desktop/minyweb_sever/%s",url);
	struct stat my_file;
...
{
	bzero(url,strlen(url));
	strcpy(path_400,"not_found_request.html");
	url = path_404;
	bzero(filename,sizeof(filename));
	sprintf(filename,"/home/zhangxingqi/Desktop/minyweb_sever/%s",url);
	struct stat my_file;
...
void http_coon::dynamic(char *filename,char *argv)
{
	...
	m_flag = true;
	bzero(request_head_buf,sizeof(request_head_buf));
	sccanf(argv,"a=%d&b=%d",&number[0],&number[1]);
...
http_coon::HTTP_CODE http_coon::head_analyse(char *temp)
{
	...
	}else if(strcasecmp(temp,"Connection:",11) == 0)
	{
		temp = temp + 11;
		while(*temp == ' ')
		{
	...
	else if(strcasecmp(temp,"Content-Length:",15) == 0)
	{
		temp = temp +15;
		while(*temp == ' ')
		{
...
{
	char path[100] = "/home/zhangxingqi/Desktop/minyweb_sever";
	char* ch;
	if(ch = strchr(url,'?'))
	{
		argv = ch +1;
